...
### Q7：控制台出现 `toLowerCase` / `content_main.js` / `404`

| 现象 | 是否项目问题 | 说明 |
|------|--------------|------|
| `content_main.js` + `toLowerCase` | ❌ 否 | Chrome **浏览器扩展**脚本，与本系统无关；可无痕模式或关掉扩展验证 |
| `GET /` 或 `/json/version` 404 | ❌ 否 | 开发工具探测后端根路径，后端只有 `/api/...` |
| `POST /api/imports` 400 | ✅ 是 | Excel 校验失败（多为表头不对） |

导入失败时，页面会弹出后端返回的中文原因（例如「第 X 列表头应为…」）。  
请按 `需求相关/分类平台表头.xlsx` 的 **19 列顺序与名称** 准备文件；末尾可多一列「操作员」。
