        ...
        <div class="card-header">
          ...
          <div>
            ...
            <el-button
              type="danger"
              :disabled="!selectedIds.length"
              @click="openRejectDialog(selectedIds)"
            >
              批量驳回
            </el-button>
          </div>
      ...
      <div class="table-wrap">
        ...
        >
          ...
          <el-table-column label="操作" width="140" fixed="right">
            <template #default="{ row }">
              <el-button link type="success" size="small" @click="handleApproveOne(row)">
                通过
              </el-button>
              <el-button link type="danger" size="small" @click="openRejectDialog([row.id])">
                驳回
              </el-button>
            </template>
          </el-table-column>
...
async function loadList() {
  loading.value = true
  try {
    const res = await listPendingReviewsApi(buildQueryParams())
...
async function handleApproveOne(row) {
  await ElMessageBox.confirm(`确认通过「${row.product_name || row.product_id}」？`, '审核通过', {
    type: 'warning',
    confirmButtonText: '通过',
    cancelButtonText: '取消',
  })
  const res = await approveReviewsApi([row.id])
  ElMessage.success(res.message)
  await loadList()
}

async function handleBatchApprove() {
  await ElMessageBox.confirm(`确认通过选中的 ${selectedIds.value.length} 条任务？`, '批量通过', {
    type: 'warning',
    confirmButtonText: '通过',
    cancelButtonText: '取消',
  })
  const res = await approveReviewsApi(selectedIds.value)
  ElMessage.success(res.message)
  await loadList()
}
...
async function handleRejectConfirm() {
  if (!rejectTargetIds.value.length) {
    ElMessage.warning('请选择要驳回的任务')
    return
  }
  rejecting.value = true
  try {
    const res = await rejectReviewsApi(rejectTargetIds.value, rejectReason.value)
    ElMessage.success(res.message)
