...
| 任务流程 | 分配、填表、暂存、提交、审核；列表支持按大类/区隔/批次号筛选；填写时大类单选、其余分类字段可多选；辅材质/包装/尺寸/卷数/总入数支持「可选也可手输」；规则选项懒加载+同路径缓存；规则提示词悬停可看全文；不展示宝贝ID，主图为首列，左侧固定到「宝贝文描图」；主图/文描图灯箱与产品属性展示；审核中心同款筛选分页 + 单条通过/驳回 |
