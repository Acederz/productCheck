...
| 模块 | 功能 |
|------|------|
| 登录/用户 | 管理员创建账号、停用、重置密码、删除无未完成任务的操作员；操作员可自助修改密码 |
| 数据导入 | Excel 导入（可选末列「操作员」按用户名自动分配；空则未分发）、错误/提醒报告；每次导入生成批次号；支持按批次删除 |
| 分类规则 | 规则 Excel 导入、级联查询 |
| 任务流程 | 分配、填表、暂存、提交、审核；操作员「我的任务」展示最新导入批次填写统计（总数据/待处理/已提交/已通过）；列表支持按大类/区隔/批次号筛选；填写时大类单选、其余分类字段可多选；辅材质/包装/尺寸/卷数/总入数支持「可选也可手输」；规则选项懒加载+同路径缓存；规则提示词悬停可看全文；不展示宝贝ID，主图为首列，左侧固定到「宝贝文描图」；主图/文描图灯箱与产品属性展示；审核中心同款筛选分页 + 单条通过/驳回 |
| 正式数据 | 审核通过入库（记录来源批次，再次通过则更新）、历史版本；支持批次号筛选；列表与导出时分类字段英文字母统一大写（不改库） |
| 导出 | 任务/正式库按条件导出 Excel；导出时对「类别」及之后字段清洗：去掉括号内说明（保留免刀撕（点断）/免刀撕（单张）），单独的「-」置空（如 盒装、- → 盒装） |
| 日志 | 操作日志、字段变更查询 |
| 系统设置 | 操作员导出开关 |
...
| POST | /api/auth/change-password | 操作员自助修改密码（需原密码；管理员 403） |
